...
algVD.randomize = function () {
  algVD.loadDefaultParams();

  // Added by Mike for less transparency
  if (Math.random() < 0.5) {
    algVD.lessTransparency = true;
  } else {
    algVD.lessTransparency = false;
  }

  // Grid Randomization
  let gridRand = Math.random();
  if (gridRand < 0.4) {
    algVD.rows = 100;
    algVD.cols = 100;
  } else if (gridRand < 0.8) {
    let temp = Math.floor(Math.random() * 101) + 50;
    algVD.rows = temp;
    algVD.cols = temp;
  } else {
    let temp1 = Math.floor(Math.random() * 51) + 75;
    let temp2 = Math.floor(Math.random() * 51) + 75;
    algVD.rows = temp1;
    algVD.cols = temp2;
  }
  algVD.maxNumOfSteps = algVD.rows * algVD.cols;
  algVD.backgroundColor = algVD.randomColor();
  if (Math.random() < 0.9) {
   algVD.backgroundColor = "#FFFFFF";
  }

  // Site Randomization
  algVD.displaySites = Math.random() < 0.25;
  algVD.siteRadius = Math.floor(Math.min(width, height) * 0.0033)
  algVD.siteFill = Math.random() < 0.9;
  algVD.sitePlacement = sitePlacementModes[Math.floor(Math.random() * sitePlacementModes.length)];
  algVD.numSites = Math.floor(Math.random() * 26) + 5;

  if (algVD.sitePlacement === "grid" || algVD.sitePlacement === "modulus") {
    algVD.siteGridDim = Math.floor(Math.random() * 7) + 2;
    algVD.numSites = algVD.siteGridDim * algVD.siteGridDim;
  }
  if (algVD.sitePlacement === "modulus") {
    if (Math.random() < 0.5) {
      algVD.sitePlacementModulusX = 2;
      algVD.sitePlacementModulusY = 2;
    } else {
      if (Math.random() < 0.5) {
        algVD.sitePlacementModulusX = Math.floor(Math.random() * 10) + 3;
        algVD.sitePlacementModulusY = Math.floor(Math.random() * 10) + 3;
      } else {
        let temp = Math.floor(Math.random() * 8) + 3;
        algVD.sitePlacementModulusX = temp;
        algVD.sitePlacementModulusY = temp;
      }
    }
  }
  if (algVD.sitePlacement === "mapToCanvas") {
    if (algVD.numSites%2 == 0) {
      algVD.numSites++;
    }
  }

  // Point Placement Randomization
  let distanceRand = Math.random();
  if (distanceRand < 0.2) {
    algVD.distanceSpace = distances[Math.floor(Math.random() * 3)];
  } else if (distanceRand < 0.6) {
    algVD.distanceSpace = distances[Math.floor(Math.random() * 6) + 3];
  } else if (distanceRand < 0.75) {
    algVD.distanceSpace = distances[Math.floor(Math.random() * 2) + 9];
  } else if (distanceRand < 0.9) {
    algVD.distanceSpace = distances[Math.floor(Math.random() * 5) + 11];
  }
  if (algVD.distanceSpace === "hyperbolic" && algVD.sitePlacement === "mapToCanvas") {
    algVD.distanceSpace = "manhattan";
  }
  if (algVD.distanceSpace === "hyperbolic" && algVD.sitePlacement === "tlbr-diagonal") {
    algVD.sitePlacement = "trbl-diagonal";
  }

  algVD.placementMode = placementModes[Math.floor(Math.random() * placementModes.length)];

  algVD.expandingSiteDistance = expandingSiteModes[Math.floor(Math.random() * expandingSiteModes.length)];
  if (Math.random() < 0.5) {
    algVD.expandingSiteDistance = "random";
  }
  algVD.expandingSitePerSite = Math.random() < 0.9;
  algVD.expandingSiteReverse = Math.random() < 0.5;

  algVD.expandShrinkDist = distances[Math.floor(Math.random() * distances.length)];
  if (Math.random() < 0.5) {
    algVD.expandShrinkx = Math.floor(Math.random() * algVD.cols);
    algVD.expandShrinky = Math.floor(Math.random() * algVD.rows);
  } else {
    algVD.expandShrinkx = -1;
    algVD.expandShrinky = -1;
  }
  ...
  if (algVD.placementMode === "random") {
    algVD.numSites = Math.floor(algVD.numSites * 1.25);
  }

  // Point Properties Randomization
  algVD.shape = shapeModes[Math.floor(Math.random() * shapeModes.length)];
  if (Math.random() < 0.5) {
    if (Math.random() < 0.5) {
      algVD.shape = "circle";
    } else {
    ...
    if (Math.random() < 0.5) {
      algVD.shape = "randomEachSite";
    }
    algVD.fill = "yes";
  }
  if (algVD.shape === "circle") {
    algVD.circleRadius = Math.floor(Math.min(width, height) * 0.0033)
  } else {
    algVD.circleRadius = Math.floor(Math.min(width, height) * 0.005)
  }
  algVD.circleRadius += Math.floor(Math.random()*3) - 1
  if (algVD.circleRadius < 1) {
    algVD.circleRadius = 1
  }
  algVD.fill = fillModes[Math.floor(Math.random() * fillModes.length)];

  algVD.varyAlphaMode = alphaModes[Math.floor(Math.random() * alphaModes.length)];
  if (Math.random() < 0.5) {
    algVD.varyAlphaMode = "fixed";
  }
  algVD.alphaSpace = distances[Math.floor(Math.random() * distances.length)];
  if (Math.random() < 0.85) {
    algVD.alphaSpace = "euclidean";
  }
  algVD.alphaMultiplier = Math.random()*0.1 + 0.9;
  if (algVD.varyAlphaMode === "fixed") {
    algVD.alphaMultiplier = 1;
  } else {
    if (Math.random() < 0.4) {
      algVD.alphaMultiplier = 0.95;
    }
  ...
  algVD.gradient = Math.random() < 0.1;
  algVD.gradientMode = gradientModes[Math.floor(Math.random() * gradientModes.length)];
  algVD.randomGradientColors = true;
