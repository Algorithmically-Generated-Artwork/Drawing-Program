...
algCL.randomize = function() {
  ...
  algCL.paletteSelect = "default";

  algCL.sinusoidalCycle = 5;
  algCL.noTrail = false;


  let randbgc = Math.random()
  if (randbgc < 0.2){
    algCL.backgroundColor = [
      Math.floor(Math.random() * 256),
      Math.floor(Math.random() * 256),
      Math.floor(Math.random() * 256)
    ];
    algCL.backgroundColor = algCL.rgbToString(algCL.backgroundColor);
  }
  else{
  ...
  let palsel = Math.floor(Math.random() * 7);
  if (palsel === 0) {
      algCL.paletteSelect = "default";
  }
  else if (palsel === 1) {
      algCL.paletteSelect = "pastel";
  }
  else if (palsel === 2) {
      algCL.paletteSelect = "earth";
  }
  else if (palsel === 3) {
      algCL.paletteSelect = "cool";
  }
  else if (palsel === 4) {
      algCL.paletteSelect = "warm";
  }
  else if (palsel === 5) {
      algCL.paletteSelect = "regal";
  }
  else {
      algCL.paletteSelect = "greyscale";
  }

  let ballnum = Math.floor(Math.random() * 100) + 1;
  algCL.numBalls = ballnum
  let colormode = Math.random()
  if (colormode < 0.2){
    algCL.gradientMethod = "none";
  }
  else if (colormode > 0.2 && colormode < 0.6){
    algCL.gradientMethod = "setlist";
  }
  else {
    algCL.gradientMethod = "inverse";
  }
  let alphalow = (Math.random() * 0.9) + 0.1;
  let alphahigh = (Math.random() * (1 - alphalow) + alphalow);
  algCL.minimumAlpha = alphalow;
  algCL.maximumAlpha = alphahigh;

  let velocitymin = (Math.random() * 9) + 1;
  let velocitymax = (Math.random() * (10-velocitymin) + velocitymin);

  algCL.minimumVelocity = velocitymin;
  algCL.maximumVelocity = velocitymax;

  let radmin = (Math.random() * 45) + 5;
  let radmax = (Math.random() * (50 - radmin) + radmin);

  algCL.minimumRadius = radmin;
  algCL.maximumRadius = radmax;

  randstepchange = (Math.random() * (algCL.maxNumOfSteps - 11)) + 10;
  algCL.randomChangeSteps = randstepchange;

  randvelinc = (Math.random() * 4) + 1;
  algCL.velocityIncrement = randvelinc;

  randballcollfactor = (Math.random() * 0.25) + 0.7;
  algCL.ballRadiiCollisionFactor = randballcollfactor;

  randcombalphchfactor = (Math.random() * 0.15) + 0.1;
  algCL.combineAlphaChangeFactor = randcombalphchfactor;

  randcollanglescale = (Math.random() * 0.2) + 0.1;
  algCL.angleRandomnessScaler = randcollanglescale;

  randsinuscycle = (Math.random() * 100) + 5;
  algCL.sinusoidalAngle = (Math.random() * (Math.PI/3)) + (Math.PI/12);

  handlecoll = (Math.random());

  if (handlecoll < 0.3){

    //wrap around time
    wraparound = Math.random();

    if (wraparound > 0.3){
      algCL.wrapAroundWalls = true;
    }
    if (wraparound > 0.1){
      algCL.wrapAroundRandom = true;
    }

    wrapstyle = Math.random();
    if (wrapstyle < 0.2){
      algCL.wrapAroundStyle = "x"
    }
    else if (wrapstyle > 0.2 && wrapstyle < 0.8){
      algCL.wrapAroundStyle = "both"
    }
    else {
      algCL.wrapAroundStyle = "y"
    }

  }
  else if (handlecoll > 0.3 && handlecoll < 0.5){
    //change velocity on wall collisions
    algCL.velocityChangesOnCollision = true;
    if (handlecoll % 2 == 0){
      algCL.speedUpOnCollision = true;
    }
  }
  else if (handlecoll > 0.5 && handlecoll < 0.8){
    algCL.ballsDivideOnWallCollision = true;
  }

  ballcoll = Math.random();
  if (ballcoll > 0.8){
    algCL.ballsCollide = true;
  }
  handleballcoll = Math.random();
  if (handleballcoll < 0.2){
    algCL.handleCollision = "none";
  }
  else if (handleballcoll > 0.2 && handleballcoll < 0.6){
    algCL.handleCollision = "combine";
    combalphmode = Math.random();
    if (combalphmode < 0.25){
      algCL.combineAlphaMode = "change";
    }
    if (combalphmode > 0.25 && combalphmode < 0.5){
      algCL.combineAlphaMode = "sum";
    }
    if (combalphmode > 0.5 && combalphmode < 0.75){
      algCL.combineAlphaMode = "min";
    }
    else{
      algCL.combineAlphaMode = "max";
    }
  }
  else if (handleballcoll > 0.6 && handleballcoll < 0.8){
    algCL.handleCollision = "disappear"
  }
  else if (handleballcoll > 0.8){
    algCL.handleCollision = "change radii";
  }

  let temp = Math.random();
  // console.log(temp);
  if (temp > 0.7){
    algCL.collisionAngleRandomness = true;
  }
  temp = Math.random();
  if (temp < 0.2){
    algCL.randomBounce = true;
  }
  temp = Math.random();
  if (temp < 0.4){
    algCL.sinusoidal = true;
  }
  temp = Math.random();
  if (temp < 0.1){
    algCL.noTrail = true;
