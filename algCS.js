...
algCS.randomize = function() {
  // Added by Mike so that the default params occur sometimes
  if(Math.random() < 0.05) {
    algCS.loadDefaultParams();
    return;
  }
  // modes
  // < 0.25: classic
  // < 0.55: varied-classic
  // < 0.85: random-pallette
  // < 0.95: random-color
  // < 1: random
  ...
  if (mode < 0.95) {
    algCS.loadDefaultParams();
    algCS.varyClassicParams();
    if (mode > 0.1) {
      algCS.varyColorPallette();
      if (mode > 0.85) {
        algCS.setRandomColorParams();
      }
    }
  } else {
    algCS.drasticRandomize();
  }

  let p = Math.random();
  if (p < 0.33) {
    algCS.backgroundColor = algCS.backgroundColors[Math.floor(Math.random()*algCS.backgroundColors.length)];
    if (Math.random() < 0.075) {

    }
  } else if (p < 0.4) {
    algCS.backgroundColor = algCS.lightBackgroundColors[Math.floor(Math.random()*algCS.lightBackgroundColors.length)];
  } else if (p < 0.45) {
    algCS.setBackground = false;
  } else {
    algCS.backgroundColor = algCS.randomHexColor();
  }
}
